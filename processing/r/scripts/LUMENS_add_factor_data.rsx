...
working_directory<-paste(tempEnv$proj_descr[3,2], "/", tempEnv$proj_descr[1,2], sep="")
setwd(working_directory)

#IMPORTING DATA INTO LUMENS DATABASE
SCIENDO1.index<-tempEnv$SCIENDO1.index+1
command="raster"
data_name<-"factor"
eval(parse(text=(paste(data_name,"_", SCIENDO1.index, "<-", command,'("', data, '")', sep=""))))
eval(parse(text=(paste(data_name,"_", SCIENDO1.index, "<-spatial_sync_raster(",data_name,"_", SCIENDO1.index, ',', 'tempEnv$ref, method = "ngb")', sep=""))))
...
eval(parse(text=(paste("tmpEnv$resave(", data_name, ",", data_description_name, ",SCIENDO1.index, file=lumens_database)", sep=""))))
