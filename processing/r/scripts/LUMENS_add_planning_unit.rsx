...
eval(parse(text=(paste("lut.pu <-read.table(lut, header=TRUE, sep=',',)", sep=""))))
eval(parse(text=(paste("names(pu_pu", pu_rec.index, ')<-"', name,'"', sep=""))))
eval(parse(text=(paste("resave(", data_name,pu_rec.index, ",lut.pu,pu_rec.index, file=lumens_database)", sep=""))))
