...
#     table.insertRow()
  ...
  def writeCSV(self):
    ...
    if savefile != '':
        ...
        rowHeader = [u'']
        rowLen = self.dataTable.rowCount()
        for h in range(rowLen):
            headColumn = self.dataTable.horizontalHeaderItem(h)
            rowHeader.append(unicode(headColumn.text()).encode('utf8'))
        writer.writerow(rowHeader)
             
        for row in range(rowLen):
            headRow = self.dataTable.horizontalHeaderItem(row)
  ...
  def writeToFile(self, filePath, encoding, driverName = 'ESRI Shapefile'): # write data to a shapefile
    ...
    with open(unicode(filePath), 'wb') as stream:
    ...
    tes = []
    tes_lagi = []
    for n in range(self.dataTable.rowCount()):
        for i in range(len(self.fields)):
            tes += [self.dataTable.item(n,i).text()]
            tes_lagi += [self.data[i][n]]
    QMessageBox.information(None,QCoreApplication.translate("CSV", "Save CSV"), \
                QCoreApplication.translate("CSV","tes = %s" % tes_lagi))
